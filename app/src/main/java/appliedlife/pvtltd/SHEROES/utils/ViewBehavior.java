...
public class ViewBehavior extends CoordinatorLayout.Behavior<HeaderView> {
    ...
    private void shouldInitProperties(HeaderView child, View dependency) {
        ...
        if (mStartMarginLeft == 0)
            mStartMarginLeft = mContext.getResources().getDimensionPixelOffset(R.dimen.dp_size_16);

        if (mEndMargintLeft == 0)
            mEndMargintLeft = mContext.getResources().getDimensionPixelOffset(R.dimen.dp_size_40);

        if (mStartMarginBottom == 0)
            mStartMarginBottom = mContext.getResources().getDimensionPixelOffset(R.dimen.dp_size_14);

        if (mMarginRight == 0)
            mMarginRight = mContext.getResources().getDimensionPixelOffset(R.dimen.dp_size_14);
