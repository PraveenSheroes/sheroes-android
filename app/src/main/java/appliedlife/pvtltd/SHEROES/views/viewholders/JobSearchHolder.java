...
public class JobSearchHolder extends BaseViewHolder<BoardingInterestJobSearch> {
    private final String TAG = LogUtils.makeLogTag(JobSearchHolder.class);
    ...
    public JobSearchHolder(View itemView, BaseHolderInterface baseHolderInterface) {
        super(itemView);
    ...
    public void bindData(BoardingInterestJobSearch item, final Context context, int position) {
        ...
        if (null != dataItem && StringUtil.isNotNullOrEmptyString(dataItem.getTitle())) {
            tvJobTag.setText(dataItem.getTitle());
        }
    }
