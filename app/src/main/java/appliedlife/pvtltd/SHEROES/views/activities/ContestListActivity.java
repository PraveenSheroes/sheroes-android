...
public class ContestListActivity extends BaseActivity implements IContestListView {
    ...
    public void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        mContestList.setLayoutManager(mLayoutManager);

        initAdapter();

        mContest = new ArrayList<>();
        mContestListPresenter.fetchContests();
    ...
    public void showError(@StringRes int stringRes) {
        Snackbar.make(mContestList, stringRes, Snackbar.LENGTH_LONG)
                .show();
    }
    ...
    private void initAdapter() {
        mContestsListAdapter = new ContestsListAdapter(this, new View.OnClickListener() {
            ...
            public void onClick(View item) {
                int position = mContestList.getChildAdapterPosition(item);
                Contest contest = mContest.get(position);
                if (CommonUtil.getContestStatus(contest.startAt, contest.endAt) == ContestStatus.UPCOMING) {
                   /* ContestPreviewActivity.navigateTo(ContestListActivity.this, contest, getScreenName(), null, CONTEST_LIST_ACTIVITY);*/
                    ...
                    ContestActivity.navigateTo(ContestListActivity.this, contest, getScreenName(), null, ContestListActivity.CONTEST_LIST_ACTIVITY, -1);
                }
            }
        });
        mContestList.setAdapter(mContestsListAdapter);
    }
