...
package appliedlife.pvtltd.SHEROES.imageoperationns;
...
public final class CropImage {
     ...
     */
    public static void startPickImageActivity(@NonNull Activity activity,int imageSelectOptionPicker) {
        activity.startActivityForResult(getPickImageChooserIntent(activity,imageSelectOptionPicker), PICK_IMAGE_CHOOSER_REQUEST_CODE);
    }

    /**
     * Same as {@link #startPickImageActivity(Activity,int) startPickImageActivity} method but instead
     ...
     */
    public static void startPickImageActivity(@NonNull Context context, @NonNull Fragment fragment,int imageSelectOptionPicker) {
        fragment.startActivityForResult(getPickImageChooserIntent(context,imageSelectOptionPicker),PICK_IMAGE_CHOOSER_REQUEST_CODE);
    }
     ...
     */
    public static Intent getPickImageChooserIntent(@NonNull Context context,int imageSelectOptionPicker) {
        return getPickImageChooserIntent(context, context.getString(R.string.ID_INTENT_CHOOSER), false, true,imageSelectOptionPicker);
    }
     ...
     */
    public static Intent getPickImageChooserIntent(@NonNull Context context, CharSequence title, boolean includeDocuments, boolean includeCamera,int imageSelectOptionPicker) {

        List<Intent> allIntents = new ArrayList<>();
        PackageManager packageManager = context.getPackageManager();

        switch (imageSelectOptionPicker) {
            ...
            default:
                ...
                List<Intent> galleryIntentsWithCamera = getGalleryIntents(packageManager, Intent.ACTION_GET_CONTENT, includeDocuments);
                if (galleryIntentsWithCamera.size() == 0) {
                    // if no intents found for get-content try pick intent action (Huawei P9).
                    galleryIntentsWithCamera = getGalleryIntents(packageManager, Intent.ACTION_PICK, includeDocuments);
                }
                allIntents.addAll(galleryIntentsWithCamera);
        }
        Intent target;
    ...
    /**
     * Get the URI of the selected image from {@link #getPickImageChooserIntent(Context,int)}.<br>
    ...
    public static ActivityBuilder activity() {
        return new ActivityBuilder(null, AppConstants.NO_REACTION_CONSTANT);
    }
    ...
    public static ActivityBuilder activity(@Nullable Uri uri,int imageSelectOptionPicker) {
        ...
        return new ActivityBuilder(uri,imageSelectOptionPicker);
    }

    /**
     * Get {@link CropImageActivity} result data object for crop image activity started using {@link #activity(Uri,int)}.
     *
    ...
    public static final class ActivityBuilder {
        ...
        private ActivityBuilder(@Nullable Uri source, int imageSelectOptionPicker) {
            mSource = source;
            mImageSelectOptionPicker=imageSelectOptionPicker;
        ...
        public Intent getIntent(@NonNull Context context, @Nullable Class<?> cls) {
            ...
            bundle.putInt(OPTION_TO_BE_OPEN, mImageSelectOptionPicker);
